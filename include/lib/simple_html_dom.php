 ...
 * Joel Carvalho - AChecker compatibility added
 * @package PlaceLocalInclude
 */
class simple_html_dom_node
{
	...
	protected function seek($selector, &$ret, $lowercase=false){
		...
		for ($i=$this->_[HDOM_INFO_BEGIN]+1; $i<$end; ++$i) {
			...
			if ($pass && $key && $val  && $val!=='*') {
				...
				//todo: MODIFIED by Joel Carvalho => &&
				if (!$check && strcasecmp($key, 'class')===0 && $exp!=='!=') {
	...
	protected function match($exp, $pattern, $value) {
		...
		switch ($exp) {
			...
			case '!=':
				// todo: BEGIN added By Joel Carvalho
				if ($pattern[0]=='/'){
					...
					$res=preg_match("/^(?:(?!".$pattern.").)*$/i", $value);
				return $res;
				// todo: END added By Joel Carvalho
				//return ($value!==$pattern); // todo: old version
			case '^=':
...
class simple_html_dom
{
	...
	protected function prepare($str, $lowercase=true, $stripRN=true, $defaultBRText=DEFAULT_BR_TEXT, $defaultSpanText=DEFAULT_SPAN_TEXT)
	{
        ...
        // Added for ACHECKER by Joel Carvalho
        $this->html = $str;
	...
	protected function read_tag()
	{
        ...
        /* customized by UOT, ATRC, cindy Li & Modified By Joel Carvalho */
        $begin_line = strrpos($this->html, $this->new_line, $begin_tag_pos+1-strlen($this->html))+1;
        $end_line = strpos($this->html, DEFAULT_BR_TEXT, $begin_tag_pos);
        if ($end_line<=$begin_line)
            $end_line = strpos($this->html, $this->new_line, $begin_tag_pos+1);
        $node->linenumber = 1;
        if (strpos(substr($this->html, 0, $end_line), $this->new_line)!==false)
            $node->linenumber = substr_count($this->html, $this->new_line, 0, $end_line)+1;
        if ($node->linenumber==0) $node->linenumber=1;
        if ($node->colnumber<=0) $node->colnumber=1;
        /* end of customized by UOT, ATRC, cindy Li & Modified By Joel Carvalho */

        $node->tag_start = $begin_tag_pos; // modified by Joel Carvalho from $begin_tag_pos to $html_pos
	...
	protected function remove_noise($pattern, $remove_tag=false)
	{
		...
		$count = preg_match_all($pattern, $this->html, $matches, PREG_SET_ORDER|PREG_OFFSET_CAPTURE); // modified to $this->html instead of $this->doc By Joel Carvalho
		...
		for ($i=$count-1; $i>-1; --$i)
		{
            ...
            /* customized by UOT, ATRC, cindy Li and Joel Carvalho*/
            $nlines=substr_count($this->noise[$key], $this->new_line);
            $new_lines_html=str_repeat($this->new_line,$nlines);
            $new_lines_doc=str_repeat(" ",$nlines);

            $this->html = substr_replace($this->html, $key.$new_lines_html, $matches[$i][$idx][1], strlen($matches[$i][$idx][0])); // For AChecker By Cindy Li and Joel Carvalho
            /* end of customized by UOT, ATRC, cindy Li and Joel Carvalho*/

            $this->doc = substr_replace($this->doc, $key.$new_lines_doc, $matches[$i][$idx][1], strlen($matches[$i][$idx][0]));
		}
