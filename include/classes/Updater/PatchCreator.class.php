...
require_once(AC_INCLUDE_PATH . "../updater/include/patch_xml_template.inc.php");
...
class PatchCreator {
     ...
     * Array
     * (
     *     [achecker_patch_id] => Patch001
     *     [achecker_version_to_apply] => 1.6
    ...
    function __construct($patch_info_array, $patch_id)
    {
        ...
        $this->patch_xml_file = AC_TEMP_DIR . "updater/patch.xml";

        $this->version_folder = AC_TEMP_DIR . "updater/" . str_replace('.', '_', $this->patch_info_array["achecker_version_to_apply"]) . "/";
        $this->patch_folder = $this->version_folder . $this->patch_info_array["achecker_patch_id"] . "/";
    ...
    function saveInfo()
    {
        ...
        if ($this->current_patch_id == 0) {
            $this->current_patch_id = $this->myownPatchesDAO->Create(
                $this->patch_info_array["achecker_patch_id"],
                $this->patch_info_array["achecker_version_to_apply"],
        ...
        if ($this->current_patch_id == 0) {
            $this->current_patch_id = $db->insert_id;
    ...
    function createXML()
    {
        ...
        // generate patch.xml
        return str_replace(array('{ACHECKER_PATCH_ID}',
            '{APPLIED_VERSION}',
            '{DESCRIPTION}',
            '{SQL}',
            '{DEPENDENT_PATCHES}',
            '{FILES}'),
            array($this->patch_info_array["achecker_patch_id"],
                $this->patch_info_array["achecker_version_to_apply"],
    ...
    function createZIP()
    {
        require_once(AC_INCLUDE_PATH . '/classes/zipfile.class.php');
        ...
        $zipfile->send_file($this->patch_info_array["achecker_patch_id"]);
    }
