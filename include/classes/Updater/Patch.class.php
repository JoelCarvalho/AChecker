<?php namespace QChecker\Updater;
/************************************************************************/
...
use QChecker\DAO\PatchesFilesActionsDAO;
use QChecker\DAO\PatchesFilesDAO;
use QChecker\Utils\SqlUtility;
...
require_once(AC_INCLUDE_PATH . "classes/DAO/PatchesFilesDAO.class.php");
require_once(AC_INCLUDE_PATH . "classes/DAO/PatchesFilesActionsDAO.class.php");
...
class Patch {
    ...
    var $svn_tag_folder = 'http://svn.atutor.ca/repos/achecker/tags/';
    var $sql_file = 'patch.sql';
    var $relative_to_AChecker_root = '../';   // relative path from updater/ to root
    ...
    {
        // add relative path to move to AChecker root folder
        for ($i = 0; $i < count($patch_array[files]); $i++) {
            ...
            $patch_array[files][$i]['location'] = $this->relative_to_AChecker_root . $patch_array[files][$i]['location'];
        }
        ...
        $this->backup_suffix = $patch_array['achecker_patch_id'] . ".old";
        $this->patch_suffix = $patch_array['achecker_patch_id'];
        $this->skipFilesModified = $skipFilesModified;

        $this->module_content_dir = AC_TEMP_DIR . "updater";
    ...
    function applyPatch()
    {
        ...
        if (count($this->backup_files) > 0) {
            ...
            $updateInfo = array("backup_files" => $db->real_escape_string($backup_files));
        }
        ...
        if (count($this->patch_files) > 0) {
            ...
            $updateInfo = array_merge($updateInfo, array("patch_files" => $db->real_escape_string($patch_files)));
        }
        ...
        if (is_array($_SESSION['remove_permission']) && count($_SESSION['remove_permission'])) {
            ...
            $updateInfo = array_merge($updateInfo, array("remove_permission_files" => $db->real_escape_string($remove_permission_files), "status" => "Partly Installed"));
        } else {
    ...
    function checkPriviledge()
    {
        ...
        foreach ($this->patch_array[files] as $row_num => $patch_file) {
            ...
            if ($real_location <> '' && !is_writable($patch_file['location']) && !in_array($real_location, $this->need_access_to_folders)) { // folder exists. check if has write permission on it
        ...
        if (count($this->need_access_to_folders) > 0 || count($this->need_access_to_files) > 0) {
            $this->errors[] = _AC('grant_write_permission');
		  ...
		  <div class="row buttons">
				<input type="submit" name="yes" value="' . _AC('continue') . '" accesskey="y" />
				<input type="submit" name="no" value="' . _AC('cancel') . '" />
    ...
    /**
     * Check if AChecker version is same as "applied version" defined in the patch.
     * @access  private
    ...
    function checkAppliedVersion()
    {
        ...
        if ($this->patch_summary_array["applied_version"] <> QCHECKER_VERSION) {
            $this->errors[] = _AC("version_not_match", $this->patch_summary_array["applied_version"]);

            $notes = '
			  <form action="' . $_SERVER['PHP_SELF'] . '?id=' . $_POST['id'] . '&who=' . $_POST['who'] . '" method="post" name="skip_files_modified">
			  <div class="row buttons">
					<input type="submit" name="ignore_version" value="' . _AC('yes') . '" accesskey="y" />
					<input type="submit" name="not_ignore_version" value="' . _AC('no') . '" />
					<input type="hidden" name="install" value="' . $_POST['install'] . '" />
					<input type="hidden" name="install_upload" value="' . $_POST['install_upload'] . '" />
				</div>
				</form>';
    ...
    function hasFilesModified()
    {
        ...
        if ($has_not_exist_files) $this->errors[] = _AC('update_local_file_not_exist') . $not_exist_files;
        if ($overwrite_modified_files) $this->errors[] = _AC('updater_overwrite_modified_files') . $overwrite_files;
        if ($alter_modified_files) $this->errors[] = _AC('updater_alter_modified_files') . $alter_files;
        if (count($this->errors) > 0) {
    ...
    /**
     * Compare user's local file with SVN backup for user's AChecker version,
     * if different, check table AC_patches_files to see if user's local file
    ...
    function isFileModified($folder, $file)
    {
        ...
        $svn_file = $this->svn_tag_folder . 'achecker_' . str_replace('.', '_', QCHECKER_VERSION) .
            str_replace(substr($this->relative_to_AChecker_root, 0, -1), '', $folder) . $file;
    ...
    {
        $this->patch_id = $this->patchesDAO->Create($patch_summary_array["achecker_patch_id"],
