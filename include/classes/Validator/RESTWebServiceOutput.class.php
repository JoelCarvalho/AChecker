...
/**
 * RESTWebServiceOutput.class.php
...
';
    ...
    function __construct($errors, $userLinkID, $guidelineArray)
    {
        ...
        $this->guidelineArray = (is_array($guidelineArray))?$guidelineArray:array();
    ...
    private function generateRESTRpt()
    {
        ...
        foreach ($this->errors as $error) { // generate each error result
            $result_type = '';
            $repair = '';
            $decision = '';
            $decision_questions = '';
            ...
            if ($row_check["confidence"] == KNOWN) { // only known errors have <repair>
                $num_of_errors++;
