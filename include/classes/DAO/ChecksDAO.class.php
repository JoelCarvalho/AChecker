...
class ChecksDAO extends DAO {
     ...
     */
    public function Create($abbr, $userID, $html_tag, $confidence,
        ...
        $html_tag = $this->db->real_escape_string(strtolower(trim($html_tag)));
        $abbr = trim($abbr);
        $note = trim($note);
        $name = trim($name);
        $err = trim($err);
        ...
        $sql = "INSERT INTO " . TABLE_PREFIX . "checks
				(`abbr`, `user_id`, `html_tag`, `confidence`, `open_to_public`, `create_date`)
				VALUES
				('" . $abbr . "'," . $userID . ",'" . $html_tag . "', '" . $confidence . "', " .
            $open_to_public . ", now())";
     ...
     * @version 1.6.3 19/09/2015
     */
    public function Update($checkID, $abbr, $userID, $html_tag, $confidence,
                           $note, $name, $err, $desc, $search_str, $long_desc,
                           $rationale, $how_to_repair, $repair_example,
                           $question, $decision_pass, $decision_fail,
                           $test_procedure, $test_expected_result,
    ...
    function setFunction($checkID, $func) {
        ...
        $sql = "UPDATE " . TABLE_PREFIX . "checks
		           SET func = '" . $this->db->real_escape_string($func) . "'
    ...
    /**
     * Filter only Open Checks from specified check ids
     * @access  public
     * @param   int[] $cids array of check IDs
     * @return  mixed
     * @author  Joel Carvalho
     * @version 1.0 02/04/2015
     */
    public function getAllOpenChecksByCheckIds($cids) {
        if (!is_array($cids)) return false;

        $sanitized_cids = Utility::sanitizeIntArray($cids);

        $sql = "SELECT distinct * FROM " . TABLE_PREFIX . "checks c
            WHERE c.open_to_public=1
            AND c.check_id in (" . implode(",", $sanitized_cids) . ")";
    ...
    /**
     * Return only Open Checks for all html elements from specified check ids
     * @access  public
     * @param   int[] $cids array of check IDs
     * @return  mixed
     * @author  Joel Carvalho
     * @version 1.0 02/04/2015
     */
    public function getOpenChecksForAllByCheckIDs($cids) {
        if (!is_array($cids) || empty($cids)) return false;

        $sanitized_cids = Utility::sanitizeIntArray($cids);

        $sql = "select distinct c.check_id, c.html_tag
						from " . TABLE_PREFIX . "checks c
						where c.check_id in (" . implode(",", $sanitized_cids) . ")
							and c.html_tag = 'all elements'
							and c.open_to_public = 1
						order by c.html_tag";
    ...
    /**
     * Return only Open Checks NOT for all html elements from specified check ids
     * @access  public
     * @param   int[] $cids array of check IDs
     * @return  mixed
     * @author  Joel Carvalho
     * @version 1.0 02/04/2015
     */
    public function getOpenChecksNotForAllByCheckIDs($cids) {
        if (!is_array($cids) || empty($cids)) return false;
    ...
    /**
     * Return prerequisite checks given specified check ids
     * @access  public
     * @param   int[] array of check IDs
     * @return  mixed
     * @author  Joel Carvalho
     * @version 1.0 02/04/2015
     */
    public function getOpenPreChecksByCheckIDs($cids) {
        if (!is_array($cids) || empty($cids)) return false;
    ...
    private function updateLang($checkID, $term, $text, $fieldName) {
        ...
        if (is_array($langs)) {// term already exists. Only need to update modified text
            if ($langs[0]['text'] <> $this->db->real_escape_string($text)) $langTextDAO->setText($_SESSION['lang'], '_check', $term, $text);
        } else {
    ...
    /**
     * check if a specified checkpoint uses hasDuplicateAttribute() function
     * @access  public
     * @param   int $cid
     * @return  boolean
     * @author  Joel Carvalho
     ...
     */
    public function hasDuplicateAttCheck($cid){
        $sql = "select check_id
            FROM ".TABLE_PREFIX."checks
            WHERE check_id = ".$cid."
            AND func LIKE '%hasDuplicateAttribute%'";
        $res=$this->execute($sql);
        return (isset($res) && count($res[0])>0);
    }

    /**
     * Return CheckID from specified check abbreviation
     * @access  public
     * @param   string $abbr Check Abbreviation
     * @return  table rows
     * @author  Joel Carvalho
     * @version 1.6.3 19/09/2015
     */
    function getCheckIdByAbbr($abbr) {
        $sql = "SELECT check_id FROM " . TABLE_PREFIX . "checks WHERE abbr='".trim($abbr)."'";
        $res = $this->execute($sql);
        return ($res[0]['check_id']);
    }
}
