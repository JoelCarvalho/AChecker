...
class GuidelineGroupsDAO extends DAO {
    ...
    public function Create($guidelineID, $name, $abbr, $principle) {
        ...
        $name = trim($name);
        $abbr = $this->db->real_escape_string(trim($abbr));
        $principle = $this->db->real_escape_string(trim($principle));
    ...
    private function updateLang($groupID, $term, $text, $fieldName) {
        ...
        if (is_array($langs)) {// term already exists. Only need to update modified text
            if ($langs[0]['text'] <> $this->db->real_escape_string($text)) $langTextDAO->setText($_SESSION['lang'], '_guideline', $term, $text);
        } else {
    ...
    /**
     * Get Group from specified abbreviation
     * @access      public
     * @param       string $abbr Group Abbreviation
     * @return      mixed return retrieved rows, true for non-select sql and false if fail
     * @author      Joel Carvalho
     * @version     1.6.3 20/09/2015
     */
    public function getGroupByAbbr($abbr){
        $sql = "SELECT * FROM " . TABLE_PREFIX . "guideline_groups
                 WHERE abbr LIKE '".trim($abbr)."'";

        return $this->execute($sql);
    }

    /**
     * Get Group from specified abbreviation and guideline ID
     * @access      public
     ...
     * @version     1.6.3 20/09/2015
     */
    public function getGroupByHierarchyAbbr($guidelineID, $abbr){
        $guidelineID = intval($guidelineID);
        $sql = "SELECT * FROM " . TABLE_PREFIX . "guideline_groups
                 WHERE abbr LIKE '".trim($abbr)."'
                 AND guideline_id = ".$guidelineID;
