...
class GuidelinesDAO extends DAO {
    ...
    public function Create($userID, $title, $abbr, $long_name, $published_date, $earlid, $preamble, $status, $open_to_public) {
        ...
        $earlid = $this->db->real_escape_string(trim($earlid));
        $preamble = $this->db->real_escape_string(trim($preamble));
    ...
    public function getGuidelineByAbbr($abbr) {
        $abbr = $this->db->real_escape_string($abbr);
    ...
    /**
     * Return Enabled Open-To-Public guidelines
     * @access  public
     * @return  mixed
     * @author  Joel Carvalho
     * @version 1.0 27/03/2015
     */
    public function getEnabledOpenGuidelines() {
        $sql = "select *
				from " . TABLE_PREFIX . "guidelines
				where open_to_public = 1 and status = 1
				order by title";
     ...
     * @version 1.0 27/03/2015
     */
    public function getEnabledGuidelines() {
        $sql = "select *
				from " . TABLE_PREFIX . "guidelines
				where status = 1
				order by title";
    ...
    /**
     * Return Open-To-Public Customized guidelines
     * @access  public
     * @return  mixed
     * @author  Joel Carvalho
     * @version 1.0 27/03/2015
     */
    public function getOpenCustomizedGuidelines() {
        $sql = "select *
				from " . TABLE_PREFIX . "guidelines
				where user_id <> 0 and open_to_public = 1
				order by title";
