...
include_once(AC_INCLUDE_PATH . 'classes/Language/Language.class.php');
...
class LanguageEditor extends Language{
    ...
    function updateTerm($variable, $term, $text)
    {
        ...
        $code = $this->db->real_escape_string($this->getCode());
    ...
    function insertTerm($variable, $key, $text, $context){
        ...
        $text = $this->db->real_escape_string($text);
        $code = $this->db->real_escape_string($this->getCode());
        $context = $this->db->real_escape_string($context);
    ...
    function printTerms($terms){
        ...
        $terms = unserialize(stripslashes($this->db->real_escape_string($terms)));
...
//		$replace = array('\"', "\'", '\0', '\n', '\r', '\Z');
        ...
        if (is_array($rows)) {
            foreach ($rows as $row) {
                $row['text'] = $this->db->real_escape_string($row['text']);
                $row['context'] = $this->db->real_escape_string($row['context']);
