...
if ($_POST['create'] || $_POST['save']) {
	...
	if (!isset($_POST["achecker_patch_id"]) || trim($_POST["achecker_patch_id"]) == "")
		$missing_fields[] = _AC("achecker_update_id");

	if (!isset($_POST["achecker_version_to_apply"]) || trim($_POST["achecker_version_to_apply"]) == "")
	...
	if (is_array($_POST['rb_action']) && $_POST['create']) {
		foreach ($_POST['rb_action'] as $i=>$action) {
			...
			if ($action == "overwrite" && $_FILES['overwrite_upload_file']['name'][$i] == "" && $_POST['overwrite_uploaded_file'] == "")
				$missing_fields[] = _AC("upload_file") . " for ". _AC("file_name") . " <strong>" . $_POST['overwrite_filename'][$i] . "</strong>";
		}
	}
	...
	if (!$msg->containsErrors()) {
		...
		require_once(AC_INCLUDE_PATH."classes/Updater/PatchCreator.class.php");
